...
class DBController:
    ...
    def getClusterCoordinatesForHashtag(self, hashtag):
        
        # select textlowercase, belongstoclusterid, coordinates2d 
        # FROM hashtag 
        # WHERE belongstoclusterid = 
        # (SELECT belongstoclusterid FROM hashtag WHERE textlowercase='votetrump2016');

        self.cursor.execute("SELECT {0}, {1}, {2} FROM {3} WHERE {1} = (SELECT {1} FROM {3} WHERE {0}='{4}')".format(
            Contract.TEXT_LOWER_CASE_COLUMN, Contract.BELONGS_TO_CLUSTER_ID, 
            Contract.COORDINATES_2D_COLUMN, Contract.TABLE_HASHTAG, hashtag))
    ...
    def getEdgeSizesForHashtag(self, hashtag):

        # SELECT hashtag1, hashtag2, edgewidth 
        # FROM representationedge, hashtag 
        # WHERE (textlowercase = hashtag1 OR textlowercase=hashtag2) 
        # AND (SELECT belongstoclusterid FROM hashtag WHERE textlowercase = hashtag1) 
        # = (SELECT belongstoclusterid FROM hashtag WHERE textlowercase='votetrump') 
        # AND (SELECT belongstoclusterid FROM hashtag WHERE textlowercase = hashtag2) 
        # = (SELECT belongstoclusterid FROM hashtag WHERE textlowercase = 'votetrump');



        self.cursor.execute("SELECT {0}, {1}, {2} FROM {3}, {4} WHERE ({5} = {0} OR {5} = {1} ) AND (SELECT {6} FROM {4} WHERE {5} = {0}) = (SELECT {6} FROM {4} WHERE {5} = '{7}') AND (SELECT {6} FROM {4} WHERE {5} = {1}) =(SELECT {6} FROM {4} WHERE {5} = '{7}') ".format(
                Contract.HASHTAG1_COLUMN, Contract.HASHTAG2_COLUMN, Contract.EDGE_WIDTH_COLUMN,
                Contract.TABLE_REPRESENTATION_EDGE, Contract.TABLE_HASHTAG, 
                Contract.TEXT_LOWER_CASE_COLUMN, Contract.BELONGS_TO_CLUSTER_ID, hashtag))
