...
class Cluster:
    ...
    @staticmethod
    def fromDBtoJSON(hashtag=None):
        dbc = DBController()
       
        print(hashtag)

        if hashtag:
            raw_nodes = dbc.getClusterCoordinatesForHashtag(hashtag)
            raw_edges = dbc.getEdgeSizesForHashtag(hashtag) 
        else:
        ...
        # use only one color if hashtag is specified
        if hashtag and len(raw_nodes)>0:
            cluster_colors[raw_nodes[0][1]] = colors[0]
